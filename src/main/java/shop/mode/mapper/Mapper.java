...
public class Mapper extends Mapper4ORM {
    ...
    public List selectBySql(Object example) throws Exception {
        List result = (List) mapper.getClass().getMethod("selectBySql", example.getClass()).invoke(mapper, example);
        for (int i = 0; i < result.size(); i++) {
            Object item = result.get(i);
            fillOnReading(item, defaultTraversalDepth);
            result.set(i, item);
        }
        return result;
    }
    public int deleteById(Integer id)throws Exception{
        return (int) mapper.getClass().getMethod("deleteByPrimaryKey", id.getClass()).invoke(mapper, id);
    }
}
