...
import shop.util.GenericsUtils;
...
public class Mapper4ORM {
    ...
    public void fillManyToOneOnReading(Object object, int depth) throws Exception {
        ...
        for (Field field : result) {
            String PColumn = field.getAnnotation(JoinColumn.class).pName();
            //获取外键的表名
            ...
            //获取外键值
            Object joinColumnValue = clazz.getMethod("get" + StringUtils.capitalize(PColumn)).invoke(object);
            ...
            // 配置criteria
            criteria.getClass().getMethod("and" + StringUtils.capitalize(joinColumn) + "EqualTo", joinColumnValue.getClass()).invoke(criteria, joinColumnValue);
            //查询,获取结果列表
